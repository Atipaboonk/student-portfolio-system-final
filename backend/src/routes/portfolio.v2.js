...
router.post("/v2", auth, uploadPortfolioV2, async (req, res) => {
  try {
    ...
    if (!yearOfProject)
      return res
        .status(400)
        .json({ message: "Year of project is required" });
    if (!category)
    ...
    if (!Number.isInteger(yearNum) || yearNum < 1900 || yearNum > 3000) {
      return res
        .status(400)
        .json({ message: "Year of project must be a valid year" });
    }
    ...
    if (imgFiles.length > 10) {
      return res
        .status(400)
        .json({ message: "Maximum 10 images allowed" });
    }
    ...
    return res
      .status(201)
      .json({ message: "Draft saved", data: portfolio });
  } catch (err) {
    ...
    if (err.code === "LIMIT_FILE_SIZE") {
      return res
        .status(400)
        .json({ message: "Each image must be ≤ 10MB" });
    }
  ...
  try {
    ...
    if (!Array.isArray(p.images) || p.images.length < 1) {
      return res.status(400).json({
        message: "At least one image is required before submit",
      });
    }

    if (p.statusV2 === "Rejected") {
